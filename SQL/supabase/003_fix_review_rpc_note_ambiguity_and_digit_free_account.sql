...
-- Mint a per-user, digit-free (letters-only) account_number. The UI surfaces
-- the wallet address instead of this value, but it must contain NO digits.
-- Replaces the previous `MV` + 8 random digits generation.
...
begin
  acct := 'MV';
  for i in 1..8 loop
    acct := acct || chr(97 + floor(random() * 26)::int);  -- 'a'..'z', no digits
  end loop;
